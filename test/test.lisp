...
(deftest test-cover-dtype/or ()
  ...
  (is (set-equal-p (dref::cover-dtype '(or t))
                   (dref::minimize-cover (lisp-locative-types))))
  (is (set-equal-p (dref::cover-dtype '(or top))
                   (dref::minimize-cover (locative-types))))
...
(deftest test-cover-dtype/and ()
  (is (set-equal-p (dref::cover-dtype '(and))
                   (dref::minimize-cover (locative-types))))
  (is (set-equal-p (dref::cover-dtype '(and nil)) '()))
  (is (set-equal-p (dref::cover-dtype '(and pseudo))
                   (dref::minimize-cover (pseudo-locative-types))))
  (is (set-equal-p (dref::cover-dtype '(and t))
                   (dref::minimize-cover (lisp-locative-types))))
  (is (set-equal-p (dref::cover-dtype '(and top))
                   (dref::minimize-cover (locative-types))))
  (signals (error :pred "Invalid DREF:DTYPE")
...
(deftest test-cover-dtype/not ()
  ...
  (is (set-equal-p (dref::cover-dtype '(not nil))
                   (dref::minimize-cover (locative-types))))
  ...
  (is (set-equal-p (dref::cover-dtype '(not class))
                   (dref::minimize-cover
                    (set-difference (locative-types)
                                    (dref::extended-locative-subtypes
                                     'class)))))
...
(deftest test-cover-dtype/member ()
  ...
  (is (set-equal-p (dref::cover-dtype '(not (member)))
                   (dref::minimize-cover (locative-types))))
...
(deftest test-cover-dtype/satisfies ()
  ...
  (is (set-equal-p (dref::cover-dtype '(satisfies f))
                   (dref::minimize-cover (locative-types))))
  (is (set-equal-p (dref::cover-dtype '(not (satisfies f)))
                   (dref::minimize-cover (locative-types)))))

...
(deftest test-doc ()
  ...
  (when (eq (backend) :swank)
    ...
    (signals-not (pax:transcription-error :handler #'continue)
      (dref::update-dref-readmes :output-dir "test/data/"))
    ...
    (check-files-the-same
     ...
     (asdf:system-relative-pathname "dref" "test/data/README"))
    #+sbcl
    (check-files-the-same
     (asdf:system-relative-pathname "dref" "README.md")
     (asdf:system-relative-pathname "dref" "test/data/README.md"))))
